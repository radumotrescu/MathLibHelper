...
void african_head()
{
	...
	for (auto i = 0; i < model.nfaces(); i++)
	{
		...
		for (int j = 0; j < 3; j++)
		{
			...
			screenCoords[j] = Vec2i({ static_cast<int>((world_coords[j].X() + 1.)*width / 2.), static_cast<int>((world_coords[j].Y() + 1.)*height / 2.) });
		}

		auto n = (world_coords[2] - world_coords[0]).Cross(world_coords[1] - world_coords[0]);
		n.Normalize();
...
Vec3f barycentric(Vec2i p1, Vec2i p2, Vec2i p3, Vec2i P)
{
	...
	*/
	...
	auto u = Vec3f{
		 {0. + p2.X() - p1.X(), 0. + p3.X() - p1.X(), 0. + p1.X() - P.X()
		} }.Cross(Vec3f
			{ 0. + p2.Y() - p1.Y(),  0. + p3.Y() - p1.Y(),  0. + p1.Y() - P.Y()
			});
	if (std::abs(u.Z()) < 1)
		return { -1., 1., 1. };
...
void triangle_tests()
{
	...
	auto t1 = std::vector<Vec2i>{ Vec2i{0, 2},   Vec2i{0, 5},  Vec2i{10, 0} };
